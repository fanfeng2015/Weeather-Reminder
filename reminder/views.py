...
def manage(request):
   ...
   if request.method == 'POST':
       ...
       if post_form.is_valid():
           ...
           Reminder.objects.create(user_id=user_id, zipcode=zipcode,
                                   warning_event=reminder)
...
def generate_warnings(data):
    ...
    try:
        ...
        if (float(tomorrow_weather['low']) - float(today_weather['low']) <= -3 or
            ...
            warnings[
                Reminder.TEMPDROP3F] = warning_text + " The temperature will drop by more than 3 degrees Fahrenheit, please wear warmer clothes."
        if (float(tomorrow_weather['low']) - float(today_weather['low']) >= 3 or
                        float(tomorrow_weather['high']) - float(today_weather['high']) >= 3):
            warnings[Reminder.TEMPRISE3F] = warning_text + " The temperature will rise by more than 3 degrees Fahrenheit."
    except:
        print(traceback.format_exc())
    return warnings
